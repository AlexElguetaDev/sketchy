...
update_track() {
    ...
    if [[ -z $SPOTIFY_JSON ]]; then
        sketchybar --set $NAME background.color=0xff666666 label.drawing=no
        return
    fi
    ...
    if [ $PLAYER_STATE = "Playing" ]; then
        ...
        sketchybar --set $NAME label="${TRACK}  ${ARTIST}" label.drawing=yes background.color=0xff999999

    elif [ $PLAYER_STATE = "Paused" ]; then
        sketchybar --set $NAME background.color=0xff666666
